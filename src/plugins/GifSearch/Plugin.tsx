...
export interface GifSearchOptions {
  ...
  onImageSearchSelectorRequest: (callbacks: {
    onSearchRequest: (searchTerm: string) => Promise<GifSearchResponseObject[]>;
...
>({
  ...
  addCommands() {
    return {
      ...
      //   ) =>
      //   () => {
      //     if (!this.options.tenorAPIKey || searchTerm.length < 1) {
      //       return false;
      //     }
      ...
      searchGifs: () => () => {
        this.options.onImageSearchSelectorRequest({
          ...
          onSelectElement: (selectedElement) => {
            this.editor.commands.setGif(selectedElement);
          },
        });
        return true;
      },
